...
// matches <foo>...</foo> extracts content
    ...
    function parse_ext_props(data, p, opts) {
        ...
        if (q.HeadingPairs && q.TitlesOfParts) {
            ...
            var parts = parseVector(q.TitlesOfParts, opts).map(function (x) {
                return x.v;
            });
            var idx = 0, len = 0;
...
//	if(s + length != blob.l) throw new Error("TxO " + (s + length) + ", at " + blob.l);
	...
	*/
    ...
    function parse_ds_bin(data, opts, rels, wb, themes, styles) {
        return {'!type': 'dialog'};
    }

    function parse_ds_xml(data, opts, rels, wb, themes, styles) {
        return {'!type': 'dialog'};
    }

    function parse_ms_bin(data, opts, rels, wb, themes, styles) {
        return {'!type': 'macro'};
    }

    function parse_ms_xml(data, opts, rels, wb, themes, styles) {
        return {'!type': 'macro'};
    }
        ...
        for (var j = 0; j != target.length; ++j) {
            ...
            for (var i = 0; i != defaults.length; ++i) {
                ...
                if (w[z[0]] == null) w[z[0]] = z[1];
                    ...
                    case "bool":
                        if (typeof w[z[0]] == "string") w[z[0]] = parsexmlbool(w[z[0]], z[0]);
                        break;
                    case "int":
    ...
    function push_defaults(target, defaults) {
        for (var i = 0; i != defaults.length; ++i) {
            ...
            if (target[z[0]] == null) target[z[0]] = z[1];
                ...
                case "bool":
                    if (typeof target[z[0]] == "string") target[z[0]] = parsexmlbool(target[z[0]], z[0]);
                    break;
                case "int":
            ...
            switch (strip_ns(y[0])) {
                ...
                case '<workbookPr/>':
                    WBPropsDef.forEach(function (w) {
                        ...
                        switch (w[2]) {
                            case "bool":
                                wb.WBProps[w[0]] = parsexmlbool(y[w[0]], w[0]);
                                break;
                            case "int":
...
//var xlmlregex = /<(\/?)([a-z0-9]*:|)(\w+)[^>]*>/mg;
    ...
    function parse_xlml(data, opts) {
        ...
        switch (opts.type || "base64") {
            ...
            case "array":
                return parse_xlml_xml(arr2str(data), opts);
        }
    }
        ...
        return function pcx(d, _opts) {
            ...
            while ((Rn = xlmlregex.exec(str))) switch ((Rn[3] = Rn[3].replace(/_.*$/, ""))) {
                ...
                case 'table-row-group':
                    break; // 9.1.9 <table:table-row-group>
